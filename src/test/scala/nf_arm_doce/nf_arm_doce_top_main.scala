...
class BFS_ps(AXI_ADDR_WIDTH : Int = 64, AXI_DATA_WIDTH: Int = 64, AXI_ID_WIDTH: Int = 6, AXI_SIZE_WIDTH: Int = 3) extends Module{
  ...
  val controls = Module(new controller(AXI_ADDR_WIDTH, 16+2,
  ...
  val Applys = Seq.tabulate(16)(
    i => Module(new Scatter(4, i, 64, 2, 16))
  )
  val Gathers = Module(new Gather(64, 4))
  val LevelCache = Module(new Apply(AXI_ADDR_WIDTH, AXI_DATA_WIDTH, AXI_ID_WIDTH + 1, AXI_SIZE_WIDTH, 2))
  val Scatters = Seq.tabulate(4)(
    i => Module(new Broadcast(64, 16, 6, 3,
      14, 5, i, 2))
  )
  val Broadcaster = Module(new Remote_xbar(16, 16, 4, 2))
  val ReApplys = Seq.tabulate(2)(
    x => Module(new Remote_Apply(64, 4, 2, x)
  ))
  ...
  ReSwitch.io.level_base_addr := VecInit(Seq.tabulate(2)(
